...
int FEM::Input() // чтение данных
{
    ...
    for (int i = 0; i < Nz; i++)
    {
        for (int k = 0; k < Nx - 1; k++)
        {
            for (int j = 0; j < Ny; j++)
            {
                all_edges[i]._1.x = X[k];
                all_edges[i]._1.y = Y[j];
                all_edges[i]._1.z = Z[i];
                all_edges[i]._2.x = X[k + 1];
                all_edges[i]._2.y = Y[j];
                all_edges[i]._2.z = Z[i];
            }
        }
    }*/
    all_edges.resize(N);
...
int FEM::SolveTask(std::vector<double> &res)
{
    ...
    // x2 = 2, z2 = 2, y2 = 2
    // hx=hy=hz=1
    // A(x,y,z)_x = q0(y2-y)(z2-z)+q1(y-y1)(z2-z)+q3(y2-y)(z-z1)+q4(y-y1)(z-z1) 
    // A(x,y,z)_y = q9(x2-x)(z2-z) q10(x-x1)(z2-z) q13(x2-x)(z-z1) q14(x-x1)(z-z1)
    // A(x,y,z)_z = q21(x2-x)(y2-y) q22(x-x1)(y2-y) q24(x2-x)(y-y1) q25(x-x1)(y-y1)
    /*double A_x = res[0]*0.25+res[1] * 0.25 + res[3]*0.25+res[4] * 0.25,
        A_y = res[9] * 0.25 + res[10] * 0.25 + res[13] * 0.25 + res[14] * 0.25,
        A_z = res[21] * 0.25 + res[22] * 0.25 + res[24] * 0.25 + res[25] * 0.25;*/

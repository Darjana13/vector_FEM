...
Solver::Solver(std::vector<int> _ia, std::vector<int> _ja,
    std::vector<double> _di, std::vector<double> _al, std::vector<double> _b)
{
    N = _di.size();
    maxIter = 10000;
    eps = 1E-15;
...
{
    if (x.size() != x0.vect.size())
        x.resize(x0.vect.size());
    for (int i = 0; i < N; i++)
        x[i] = x0.vect[i];
}
